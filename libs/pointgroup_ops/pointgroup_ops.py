...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        pointgroup_ops_cuda.voxelize_idx(coords, output_coords, input_map, output_map, batchsize, mode)
...
class Voxelization(Function):
    ...
    def forward(ctx, feats, map_rule, mode=4):
        ...
        pointgroup_ops_cuda.voxelize_fp(feats, output_feats, map_rule, mode, M, maxActive, C)
        return output_feats
    ...
    def backward(ctx, d_output_feats):
        ...
        pointgroup_ops_cuda.voxelize_bp(d_output_feats.contiguous(), d_feats, map_rule, mode, M, maxActive, C)
