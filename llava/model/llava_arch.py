...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        #     self.config.mm_hidden_size = self.config.mm_hidden_size + model_args.pose_encode_dim
        #     self.config.pose_encode_dim = getattr(model_args, 'pose_encode_dim', None)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def visibility_check_simple(self, target_points, fx, fy, cx, cy, width, height, T_wc):
        T_cw = torch.linalg.inv(T_wc.to(torch.float32))
        Pw = torch.hstack([target_points, torch.ones((len(target_points),1), device=target_points.device)])
        Pc = (T_cw.to(Pw.dtype) @ Pw.T).T
        Xc, Yc, Zc = Pc[:,0], Pc[:,1], Pc[:,2]

        u = (fx * Xc/Zc + cx) # normally this should be type int, but we keep it float for later feature processing
        v = (fy * Yc/Zc + cy)

        mask = (Zc > 0) & (u>=0) & (u<width) & (v>=0) & (v<height)
        return Xc, Yc, Zc, u, v, mask
    
    def encode_camera_extrinsic_in_coords(self, world2cam, coord):
        assert world2cam is not None, "world2cam should not be None"

        fx, fy, cx, cy = 1169.621094, 1167.105103, 646.295044, 489.927032
        width, height = 1296, 968

        Xc, Yc, Zc, u, v, mask = self.visibility_check_simple(
            coord, fx, fy, cx, cy, width, height, world2cam
        )
        u_feats, v_feats = mask * (u / width), mask * (v / height) # normalized image coordinates for in-image points.
        u_feats[~mask], v_feats[~mask] = -1.0, -1.0 # -1 for out of image points
        stacked_feats = [Xc.unsqueeze(1), Yc.unsqueeze(1), Zc.unsqueeze(1), 
                         u_feats.unsqueeze(1), v_feats.unsqueeze(1), 
                         mask.to(torch.float32).unsqueeze(1)]

        encoded_coords = torch.cat(stacked_feats, dim=1).to(coord.device)
        return encoded_coords

    def encode_pointclouds(
        self, coord, grid_coord, offset, feat, p2v_map, v2p_map, spatial_shape, superpoint_mask, prompt_mask, possible_pose=None
        ):
        sampled_features, prompt_features, superpoint_features, mask_input_dict, topk_query_coord = self.get_model().get_pointcloud_tower()(
            coord, grid_coord, offset, feat, p2v_map, v2p_map, spatial_shape, superpoint_mask, prompt_mask
        )

        if possible_pose is not None:
            # encode pose information into additional dimensions of point features.
            stacked_sample_feats_coords = [torch.cat([sampled_features[idx], self.encode_camera_extrinsic_in_coords(possible_pose[idx], topk_query_coord[idx]).to(sampled_features[idx].dtype).to(sampled_features[idx].device)], dim=-1)
                                            for idx in range(len(sampled_features))]
            # recover the pose encoding dimension adaptively, without need for parameter setting
            pose_encode_dim = stacked_sample_feats_coords[0].shape[-1] - sampled_features[0].shape[-1]
            # stuff zeros here as well to align dimension sizes. Changes nothing.
            stacked_prompt_feats_coords = [torch.cat([prompt_features[idx], torch.zeros(prompt_features[idx].shape[0], pose_encode_dim, device=prompt_features[0].device).to(prompt_features[idx].dtype)], dim=-1)
                                           for idx in range(len(prompt_features))]
        else:
            stacked_sample_feats_coords = sampled_features
            stacked_prompt_feats_coords = prompt_features

        pointcloud_tokens = [self.get_model().mm_projector(feat) for feat in stacked_sample_feats_coords]
        prompt_tokens = [self.get_model().mm_projector(feat) for feat in stacked_prompt_feats_coords]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        click_mask=None, possible_pose=None, **kwargs
    ):  
        pointcloud_tower = self.get_pointcloud_tower()
        ...
        pc_tokens, prompt_tokens, superpoint_features, mask_input_dict = self.encode_pointclouds(
             coord, grid_coord, offset, pc_input, p2v_map, v2p_map, spatial_shape, superpoint_mask, click_mask, possible_pose
        )
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                cur_input_embeds = torch.cat([cur_input_embeds_1, cur_pc_features[0:0]], dim=0)  # empty slice keeps tensor on graph
