...
def compute_iou(pred_masks, true_mask):
    """
    Compute IoU between predicted masks and the ground-truth mask.

    Args:
        pred_masks: Predicted masks, shape (N, M).
        true_mask: Ground-truth mask, shape (1, M).

    Returns:
        iou: IoU for each predicted mask vs ground-truth, shape (N, 1).
    """
