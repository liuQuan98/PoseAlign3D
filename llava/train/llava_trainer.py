...
def get_per_batch_task_length_grouped_indices(lengths, batch_size, world_size, generator=None):
    """
    Each batch contains a single task; each mega batch (batch_size * world_size) may mix multiple tasks.
    """
    ...
    # Shuffle within each task and split into batch_size chunks
    for task_id in task_ids:
    ...
    # Task-level batch pool
    random.shuffle(task_batches)

    # Organize mega batches; each mega batch may contain batches from different tasks
    megabatches = []
...
class LLaVATrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.training_args = kwargs.get('args', None)
        assert self.training_args is not None, "training_args must be provided in kwargs"
