...
class ModelArguments:
    ...
    apply_pose_to_projection: bool = field(default=False)
    # typically it's 6, first 3 represents camera-coordinates, next 2 represents image coordinates, 
    # last 1 represents whether object is in view frustum
    pose_encode_dim: int = 6 
...
class DataArguments:
    ...
    # additional params added for pose-related improvements
    use_cam_instance_intersect: bool = field(default=False)
    apply_pose_to_pc: bool = field(default=False)
    apply_pose_to_prompt: bool = field(default=False)
    use_top_bottom_cut: bool = field(default=False)
    use_random_pose: bool = field(default=False)
    ...
    for source in sources:
        for sentence in source:
            if sentence['from'] == 'human':
                start_idx = sentence['value'].find(DEFAULT_PC_TOKEN)
                # only add prompt after the first PC token. The token is always followed by '\n' so the index is shifted by 2
                if start_idx != -1:
                    split_index = start_idx + len(DEFAULT_PC_TOKEN) + 2
                    sentence['value'] = sentence['value'][:split_index] + additional_prompt + sentence['value'][split_index:]
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        assert not self.data_args.scene_wise, "rearranging the data in scene-wise manner breaks scanqa indexes and is therefore disabled, but temporarily supported."
    ...
    def _build_pc_transform(self):
        # apply pose to input pc
        if self.data_args.use_cam_instance_intersect and self.data_args.apply_pose_to_pc and not self.data_args.apply_pose_to_projection:
            self.rs_transform = Compose(referseg_transform_train_posealign)
            if self.data_args.scene_alignment_file is not None:
                vqa_transform_train_posealign[0]['file_path'] = self.data_args.scene_alignment_file
                densecap_transform_train_posealign[0]['file_path'] = self.data_args.scene_alignment_file
            self.vqa_transform = Compose(vqa_transform_train_posealign)
            self.densecap_transform = Compose(densecap_transform_train_posealign)
        # apply pose to projection layer
        elif self.data_args.use_cam_instance_intersect and self.data_args.apply_pose_to_projection and not self.data_args.apply_pose_to_pc:
            self.rs_transform = Compose(referseg_transform_train_collectPose)
            if self.data_args.scene_alignment_file is not None:
                vqa_transform_train_collectPose[0]['file_path'] = self.data_args.scene_alignment_file
                densecap_transform_train_collectPose[0]['file_path'] = self.data_args.scene_alignment_file
            self.vqa_transform = Compose(vqa_transform_train_collectPose)
            self.densecap_transform = Compose(densecap_transform_train_collectPose)
        # apply pose to both input pc and projection layer
        elif self.data_args.use_cam_instance_intersect and self.data_args.apply_pose_to_pc and self.data_args.apply_pose_to_projection:
            self.rs_transform = Compose(referseg_transform_train_posealign_collectpose)
            if self.data_args.scene_alignment_file is not None:
                vqa_transform_train_posealign_collectpose[0]['file_path'] = self.data_args.scene_alignment_file
                densecap_transform_train_posealign_collectpose[0]['file_path'] = self.data_args.scene_alignment_file
            self.vqa_transform = Compose(vqa_transform_train_posealign_collectpose)
            self.densecap_transform = Compose(densecap_transform_train_posealign_collectpose)
        # no pose is applied, or pose is only applied to prompt
        else:
    ...
    def encode_pose_prompt(self, prompt, pose):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        assert len(sources) == 1, "Expected single source per item"
        if 'image' in sources[0]:
            ...
            if self.data_args.use_cam_instance_intersect:
                ...
                if i > self.scanqa_idx_start and i <= self.scanqa_idx_end:
                    object_id_tmp = np.random.choice(self.extra_data_dict[i - self.scanqa_idx_start]) # supplement absent object ids for scanqa
                elif isinstance(object_id, Sequence):
                    ...
                    try:
                        ...
                        print(f"Error in choosing object_id from {object_id}, scene {scene_name}, is_scanqa: {i > self.scanqa_idx_start and i <= self.scanqa_idx_end}: {e}", flush=True)
                        raise e
                ...
                if self.data_args.use_random_pose:
                    pose_instance_process_function = calculate_random_camera_extrinsics
                elif self.data_args.use_top_bottom_cut:
                ...
                try:
                    possible_camera_extrinsic = pose_instance_process_function(
                        ...
                        pose_data_path_base=self.data_args.pose_data_path_base, cut_ratio=self.data_args.cut_ratio
                    )
                    ...
                    print(f"Error in calculating camera extrinsics for scene {scene_name}, object {object_id_tmp}, is_scanqa: {i > self.scanqa_idx_start and i <= self.scanqa_idx_end}: {e}", flush=True)
                    raise e
                
                if self.data_args.pose_aug_rot_std > 1e-6 or self.data_args.pose_aug_trans_std > 1e-6:
                    possible_camera_extrinsic = perturb_pose_yaw_only(possible_camera_extrinsic,
                                                                      yaw_std_rad=self.data_args.pose_aug_rot_std,
                                                                      trans_std=self.data_args.pose_aug_trans_std,
                                                                      left_multiply=False)
            ...
            if self.data_args.apply_pose_to_prompt:
                possible_pose = possible_camera_extrinsic
                encoded_pose_prompt = self.encode_pose_prompt(copy.deepcopy(DEFAULT_PC_PROMPT), possible_pose)
                conv_sources = add_pose_align_prompt(
                    conv_sources,
                    self.data_args,
                    additional_prompt=encoded_pose_prompt
                )
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'scene_id' in instances[0]:
            ...
            for d in instances:
                ...
                if 'possible_camera_extrinsic' in d:
                    possible_pose_list.append(d['possible_camera_extrinsic'])
            ...
            batch.update({
                ...
                "possible_pose": possible_pose_list if len(possible_pose_list) > 0 else None,
            })
        return batch
...
def load_partial_weights(model, old_model):
    ...
    for k, v in state_dict.items():
        if k in model_dict:
            if model_dict[k].shape == v.shape:
                ...
                # partial load: match as many dims as possible
                min_shape = tuple(min(a, b) for a, b in zip(model_dict[k].shape, v.shape))
                slices = tuple(slice(0, s) for s in min_shape)
                model_dict[k][slices] = v[slices]
                print(f"Partially loaded {k}: old {v.shape} -> new {model_dict[k].shape}")
...
def train(attn_implementation=None):
    ...
    data_args.apply_pose_to_projection = model_args.apply_pose_to_projection  # propagate model_args to data_args
    ...
    if model_args.vision_tower is not None or model_args.pointcloud_tower is not None:
        if getattr(model_args, 'apply_pose_to_projection', False):
            assert model_args.pose_encode_dim == 6, "currently only pose_encode_dim=6 is supported"
            # these additional dims encode the camera pose with coords.
            # typically it's 6, first 3 represents camera-coordinates, next 2 represents image coordinates, last 1 represents whether object is in view frustum
            config_new = copy.deepcopy(model.get_model().config)
            config_new.mm_hidden_size = config_new.mm_hidden_size + model_args.pose_encode_dim
            config_new.pose_encode_dim = model_args.pose_encode_dim
            mm_projector_new = build_vision_projector(config_new)
            # inherit weights from old projector except for the newly added pose dims
            load_partial_weights(mm_projector_new, model.get_model().mm_projector)
            model.get_model().mm_projector = mm_projector_new
            model.get_model().config = config_new
        ...
        training_args.apply_pose_to_projection = data_args.apply_pose_to_projection
    ...
    rank0_print(f"Num params: {n_parameters}, trainable: {n_parameters_trainable} ({n_parameters_trainable / n_parameters * 100:.2f}%)")
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 and model_args.apply_pose_to_projection:
            # save the mm_projector separately for easy loading
            torch.save(model.get_model().mm_projector.state_dict(), os.path.join(training_args.output_dir, 'mm_projector.bin'))
    else:
