...
def perturb_pose_yaw_only(
    ...
    *,
    ...
    yaw_rad = (np.random.rand() - 0.5) * 2 * yaw_std_rad
    delta_t = (np.random.randn(3) - 0.5) * 2 * trans_std

    rot_matrices = rotz_batch(np.array([yaw_rad]))

    R = T[:3, :3]
    t = T[:3, 3]

    rotations = rot_matrices @ R

    final_pose = np.zeros((4, 4))

    final_pose[3, 3] = 1
    final_pose[:3, :3] = rotations
    final_pose[:3, 3] = t + delta_t
    return final_pose
