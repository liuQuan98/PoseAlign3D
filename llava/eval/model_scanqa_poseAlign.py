...
from llava.pc_utils import vqa_transform_eval, vqa_transform_eval_posealign, vqa_transform_eval_collectPose, vqa_transform_eval_posealign_collectpose, Compose
from PIL import Image
from llava.mm_utils import tokenizer_special_token
import math
import pathlib
import numpy as np
from llava.train.train import DataCollatorForSupervisedDataset, add_pose_align_prompt
...
def get_possible_pose(scene_name, args, i):
    global extra_data_dict
    cam_instance_intersection_filemap = load_filemap(scene_name=scene_name)

    # this is scanqa, not need for judging scanqa based on index, just use it
    object_id_tmp = np.random.choice(extra_data_dict[i]) # supplement absent object ids for scanqa
...
def eval_model(args):
    ...
    for source in tqdm(questions):
        ...
        possible_pose = get_possible_pose(scan_file, args, idx)
        ...
        if args.apply_pose_to_pc and not args.apply_pose_to_projection:
            transform = Compose(vqa_transform_eval_posealign)
        elif args.apply_pose_to_projection and not args.apply_pose_to_pc:
            transform = Compose(vqa_transform_eval_collectPose)
        elif args.apply_pose_to_pc and args.apply_pose_to_projection:
            transform = Compose(vqa_transform_eval_posealign_collectpose)
        else:
        ...
        for key in pc_data_dict:
            if key in ["coord", "grid_coord", "feat", "offset", "possible_camera_extrinsic"]:
