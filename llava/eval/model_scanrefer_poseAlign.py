...
from llava.pc_utils import referseg_transform_eval, referseg_transform_eval_posealign, referseg_transform_eval_collectPose, referseg_transform_eval_posealign_collectpose, Compose
...
def eval_model(args):
    ...
    # tokenizer, model, _, _ = load_pretrained_model(model_path, args.model_base, model_name, pointcloud_tower_name=args.pointcloud_tower_name, ignore_mismatched_sizes=False)
    ...
    for idx, source in enumerate(tqdm(questions)):
        ...
        possible_pose = get_possible_pose(scan_file, args, object_id=object_id)
        ...
        if args.apply_pose_to_pc and not args.apply_pose_to_projection:
            transform = Compose(referseg_transform_eval_posealign)
        elif args.apply_pose_to_projection and not args.apply_pose_to_pc:
            transform = Compose(referseg_transform_eval_collectPose)
        elif args.apply_pose_to_pc and args.apply_pose_to_projection:
            transform = Compose(referseg_transform_eval_posealign_collectpose)
        else:
            transform = Compose(referseg_transform_eval)
