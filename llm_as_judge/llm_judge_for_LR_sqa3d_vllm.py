...
GRADER_TEMPLATE = """
Your job is to look at a spatial relational query regarding a room for a QA task, and then assign the problem as either [A: "NEED_LATERAL_DIRECTION", B: "DO_NOT_NEED_LATERAL_DIRECTION"] in order to answer.
...
```
Question 1: I am standing in front of the dresser under the bed putting away clothes. Can I reach the drawers in the dresser underneath the bed in front of me? Answer the question using a single word or phrase.
Answer 1: Yes
Question 2: Sitting on the chair closest to the window behind me. Is there room for another chair at the table? Answer the question using a single word or phrase.
Answer 2: Yes
```
This question is DO_NOT_NEED_LATERAL_DIRECTION because:
    - Either the description do not contain any directional information at all.
    - Or the question clearly contains no movement of ego agent in any lateral direction, e.g., we start and end at both 'in front of me' location in question 1.
    - Or the description is about other traits of the object than its direction (e.g., space, reachability, distance, color, material, shape, size, etc.).


The following are examples of question that NEED_LATERAL_DIRECTION.
```
Question 1: I am washing my face in the sink in front of the mirror. What is on the right side of the soap dispenser in front of me?
Answer 1: Mirror
Question 2: I am fixing the radiator with the desk on my right within reach. The bed is on which side of me? Answer the question using a single word or phrase.
Answer 2: right
Question 3: I am sitting on a chair with a trash can on the floor by my left leg. Is the amount of board on my left odd or even? Answer the question using a single word or phrase.
Answer 3: even
```
This question is NEED_LATERAL_DIRECTION because:
    - A relative direction word (e.g., "left", "right", "front", "back", "against") is present in either the question or the answer, and describes relative position against some other object or the agent itself.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder-name", type=str, default="./llm_as_judge/sqa3d_vllm")
    ...
    for qid in tqdm(qa_pairs.keys()):
        ...
        question, answer = qa_pairs[qid]
        text = ", ".join([f'"{p}"' for p in answer])
